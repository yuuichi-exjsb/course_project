...
public class GameMaster extends Canvas implements KeyListener {
  ...
  Image titleImg = getToolkit().getImage("img/shootingGameTitle.png");
  Image airplane = getToolkit().getImage("img/airplane2.png");
  Image backScreen = getToolkit().getImage("img/space.png");
...
for(int i=0;i<enmyBnum;i++){
  if(enmyB[i].hp >0){
    ftr.collisionCheck(enmyB[i]);
    for(int k=0;k<ftrBltNum;k++){
       if (ftrBlt[k].hp > 0)              // 自弾が生きていたら
       ftrBlt[k].collisionCheck(enmyB[i]);
    }
  }
      }
      // *** 自機の生死を判断 ***
 ...
 ftrBlt[i].move(buf_gc, imgW, imgH);
      ...
      buf_gc.drawImage(airplane,ftr.x-32,ftr.y,64,64,this);
